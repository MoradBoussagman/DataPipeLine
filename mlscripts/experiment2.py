...
EXPERIMENT = "air_quality_aqi_prediction"
...
def run_experiment():
   ...
   # ── 6. Write best model to postgres ──────────────────────
    if DATA_SOURCE == "postgres":
        conn = psycopg2.connect(
            host=PG_HOST, database=PG_DB,
            user=PG_USER, password=PG_PASS
        ...
        # is_active only if this version was promoted to production
        is_active = (str(result.version) == client.get_model_version_by_alias(MODEL_NAME, "production").version)

        if is_active:
            cur.execute("UPDATE model_registry SET is_active = FALSE")

        cur.execute("""
            INSERT INTO model_registry (mlflow_run_id, model_version, model_name, rmse, mae, is_active)
            VALUES (%s, %s, %s, %s, %s, %s)
            ON CONFLICT (mlflow_run_id) DO UPDATE SET is_active = %s
        """, (
            best_run_id,
            int(result.version),
            best_name,
            float(best_rmse),
            float(mean_absolute_error(y_test, models[best_name].predict(X_test))),
            is_active,
            is_active
        ))
        ...
        cur.close()
        conn.close()
        print(f"✓ model_registry updated → version {result.version} is_active={is_active}")
